...
namespace DZ.Tools.Tests
{
    ...
    public class HtmlParserTests
    {
        ...
        public void CanParseCorrectModel()
        {
            ...
            var model = NERWorker.I.Parser.Parse(ModelInput);
        ...
        {
            var model = NERWorker.I.Parser.Parse(ModelInput);
            var render = model.Render(NERWorker.I.Renderer);
        ...
        public void Convert()
        {
            foreach (var filePath in Directory.EnumerateFiles(@"E:\tmp\Corpora"))
            {
                ...
                var model = NERWorker.I.Parser.Parse(File.ReadAllText(filePath));
                File.WriteAllText(Path.Combine(@"E:\tmp\Corpora\res", fileName + ".txt"), model.Render(NERWorker.I.Renderer));
            }
        }
    }
}